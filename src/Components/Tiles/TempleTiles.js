...
import { startTileContainer, templeTile } from './Setup.css';
...
export const TempleTiles = (props) => {
  ...
  const columns = useMedia(['(min-width: 1500px)', '(min-width: 1000px)', '(min-width: 600px)'], [3, 3, 3], 3);
  ...
  useEffect(() => {
    const t = setInterval(() => {if (items.length >= 4) {items.pop(); set(shuffle);}}, 100);
    ...
    return (
        <div ref={ref} css={startTileContainer} style={{ height: Math.max(...heights) }}>
          ...
          {transitions((style, item) => (
            <a.img css={templeTile} style={style} src={process.env.PUBLIC_URL + "/TempleTiles/base/" + item.name + ".jpg"} data-tip={item.tooltip}/>          
            ))}
        </div>
